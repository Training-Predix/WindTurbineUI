...
if(node_env === 'development') {
  // use localConfig file
	...
	settings.acsURL = devConfig.acsURL;
	settings.acsZoneId = devConfig.acsZoneId;

} else {
	// read VCAP_SERVICES
	var vcapsServices = JSON.parse(process.env.VCAP_SERVICES);
	var uaaService = vcapsServices[process.env.uaa_service_label];
	var assetService = vcapsServices[process.env.asset_service_label];
	var timeseriesService = vcapsServices[process.env.timeseries_service_label];
	var acsService = vcapsServices[process.env.acs_service_label];
	...
	if(acsService) {
		settings.acsURL = acsService[0].credentials.uri; // + '/' + process.env.assetMachine
		settings.acsZoneId = acsService[0].credentials.zone['http-header-value'];
	}
...
settings.buildVcapObjectFromLocalConfig = function(config) {
  'use strict';
	...
	var vcapObj = {};
	if (config.uaaURL) {
		vcapObj[devConfig.uaaServiceLabel] = [{
			credentials: {
				uri: config.uaaURL
			}
		}];
	...
	if (config.timeseriesURL) {
		vcapObj[devConfig.timeseriesServiceLabel] = [{
			credentials: {
				query: {
	...
	if (config.assetURL) {
		vcapObj[devConfig.assetServiceLabel] = [{
			credentials: {
				uri: config.assetURL,
				zone: {
